...
export function handleBlock(block: ethereum.Block): void {
    ...
    if (n == 680930) {
        let _lastBlock = new lastBlock(Bytes.fromByteArray(Bytes.fromBigInt(BigInt.fromU32(680930))));
        _lastBlock.blockTimestamp = block.timestamp
        _lastBlock.blockNumber = block.number
        _lastBlock.save()
    } else if (n % 17280 == 0){
        let _lastBlock = lastBlock.load(Bytes.fromByteArray(Bytes.fromBigInt(BigInt.fromU32(680930))));
